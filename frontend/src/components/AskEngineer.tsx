...
//       if (data.answer) {
//         setMessages((prev) => [...prev, { id: Date.now().toString(), role: 'ai', text: data.answer }]);
//       } else {
//         throw new Error('No answer received');
//       }
//     } catch (error) {
//       setMessages((prev) => [...prev, { id: Date.now().toString(), role: 'ai', text: '⚠️ Comm failure. Check the pit wall connection.' }]);
//     } finally {
//       setIsTyping(false);
//     }
//   };
...
//             }`}>
//               {msg.role === 'ai' && (
//                 <div className="text-[10px] text-red-500 font-bold tracking-widest uppercase mb-2">Race Engineer</div>
//               )}
...
// import React, { useState, useRef, useEffect } from 'react';
// import { useRaceStore } from '../store/useRaceStore'; // 1. Import your Zustand store
...
//   // 2. Grab the currently selected race ID from global state
...
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         // 3. Inject the activeRaceId into the payload alongside the prompt
//         body: JSON.stringify({ 
...
export default function AskEngineer() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.answer) {
        const aiMessage: Message = { id: Date.now().toString(), role: 'ai', text: data.answer };
      ...
      const errorMessage: Message = { id: Date.now().toString(), role: 'ai', text: '⚠️ Comm failure. Check the pit wall connection.' };
